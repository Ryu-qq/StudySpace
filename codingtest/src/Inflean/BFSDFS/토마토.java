 ...
 * 며칠이 지나면 토마토들이 모두 익는지, 그 최소 일수를 구하는 프로그램을 작성하라. 단, 상자의 일부 칸에는 토마토가 들어있지 않을 수도 있다.입력
 *
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                if (board[i][j] == 1) q.add(new Pointer(i, j));
            }
        }
    ...
    public static void BFS() {
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (cx >= 0 && cy >= 0 && cx < n && cy < m && board[cx][cy] == 0) {
                    board[cx][cy] = 1;
                    q.add(new Pointer(cx, cy));
